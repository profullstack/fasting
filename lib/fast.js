...
import { getCurrentTimestamp } from './config.js';
...
export const startFast = async (startTime = null) => {
  ...
  const timestamp = startTime ? new Date(startTime).toISOString() : getCurrentTimestamp();
...
export const endFast = async (endTime = null) => {
  ...
  const endTimestamp = endTime ? new Date(endTime).toISOString() : getCurrentTimestamp();
